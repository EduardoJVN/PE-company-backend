...
export interface ActivateCompanyMemberInput {
  companyId: string;
  requesterId: string;
  targetUserId: string;
}

export class ActivateCompanyMemberUseCase {
  constructor(private readonly companyRepo: ICompanyRepository) {}

  async execute(input: ActivateCompanyMemberInput): Promise<void> {
    ...
    const reactivated = target.reactivate();

    await this.companyRepo.activateMember(reactivated);
  }
}
