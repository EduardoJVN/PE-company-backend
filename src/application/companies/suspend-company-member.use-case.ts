...
export interface SuspendCompanyMemberInput {
  companyId: string;
  requesterId: string;
  targetUserId: string;
}

export class SuspendCompanyMemberUseCase {
  constructor(private readonly companyRepo: ICompanyRepository) {}

  async execute(input: SuspendCompanyMemberInput): Promise<void> {
    ...
    await this.companyRepo.suspendMember(suspended);
  }
}
