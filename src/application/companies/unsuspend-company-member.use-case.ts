...
export interface UnsuspendCompanyMemberInput {
  companyId: string;
  requesterId: string;
  targetUserId: string;
}

export class UnsuspendCompanyMemberUseCase {
  constructor(private readonly companyRepo: ICompanyRepository) {}

  async execute(input: UnsuspendCompanyMemberInput): Promise<void> {
    ...
    const targetResult = await this.companyRepo.findMemberByUserAndCompanyAnyStatus(
      input.companyId,
      input.targetUserId,
    );
    ...
    const unsuspended = target.unsuspend();

    await this.companyRepo.unsuspendMember(unsuspended);
  }
}
